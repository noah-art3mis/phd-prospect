...
- [x] **The ingest prompt is a file — `prompts/ingest.md`, read at startup, versioned in git.** Editing it does not mean editing code, and diffs stay readable. Rejected: a template literal in source (noisy diffs, tooling reformats long prose) and a runtime-editable prompt in the database (contradicts git as source of truth, and the prompt that produced a record would exist nowhere in history).
- [x] **No prompt-version stamp on records.** Considered a `prompt_version` column holding the prompt file's short hash, so a record could be traced to the prompt generation that produced it. Not adopted. Known cost: after a few rounds of prompt tuning the corpus is a mix of generations with no marker, so "did extraction get better?" is unanswerable from the data. One column and one hash if that becomes worth knowing.
