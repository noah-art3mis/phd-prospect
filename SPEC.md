...
- **AI model:** `claude-sonnet-5` at effort `high` with adaptive thinking, for the single ingest call. Haiku is not eligible — the `web_search_20260209` / `web_fetch_20260209` tool versions require Opus 5/4.8/4.7/4.6, Sonnet 5, or Sonnet 4.6. Effort stays at `high` because the design depends on the model actually calling its tools, and tool-use rate falls at lower effort and with thinking disabled. Cost is roughly $0.12–$0.30 per opportunity. The response is constrained by **structured outputs** (`output_config.format`), so a malformed record is not a state the system can reach. Deterministic validation still runs — it enforces the evidence rule and the state machine, which a schema cannot express.
