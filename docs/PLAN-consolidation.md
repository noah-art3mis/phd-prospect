...
- ~~**Timezone in `compute-due-reminders.js`**~~ Fixed repo-side on `feat/phase4-cutover` (`TZ = 'Europe/London'`, goldens unaffected); NOT yet deployed live — live mutation of 02 needs user consent because a concurrent session built it. Deploy = apply the built `n8n/import/02-deadline-reminders.json` node changes to `8PJDDDrPkVHX284l`, then re-run the round-trip check.
- ~~**Pending-approval TTL sweep**~~ Wired repo-side on `feat/phase4-cutover` (Daily trigger → `compute-ttl-cutoff.js` (7 days) → deleteRows on `created_at lt cutoff` → per-row Telegram notify); NOT yet deployed live — same consent gate as the timezone fix.
