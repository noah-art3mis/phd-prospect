...
test('the config surface is exactly the keys the spec settled on', () => {
  // The model id and max_tokens are deliberately absent: they live in the prompt file,
  // because they are tuned together with the wording rather than varying by deployment.
